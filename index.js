...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6289514468104@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'gtts':
                    ...
                    if (args.length < 2) return client.sendMessage(from, 'Textnya mana?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
